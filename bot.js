...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'serieux' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor("Flash Story Server Status", "https://cdn.discordapp.com/attachments/933182414702911528/933187546740031528/logo.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Flash Story RP' : '⚫ Flash Story RP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nComment rejoindre le serveur ?','Vous pouvez rejoindre le serveur en allant dans **Se connecter** En bas se trouve létat du serveur pour voir combien de personnes sont en ligne et dans la file dattente. Ce système a été créé par Alex Valdo\n\u200b\n',false)
    if (STATUS !== undefined)
    {
  ...
  const offline = function() {
    ...
    .addField('file dattente','?',true)
    .addField('Online joueurs','?\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('file dattente',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Online joueurs',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    bot.user.setActivity('Flash Story Rp',{'url':'https://www.twitch.tv/alexvaldobh','type':'STREAMING'});
